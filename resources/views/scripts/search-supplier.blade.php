<script>
    $(function() {
        ...
        var usersTable = $('#supplier_table');
        var resultsContainer = $('#search_results');
        var usersCount = $('#user_count');
        var clearSearchTrigger = $('.clear-search');
        var searchform = $('#search_supplier');
        var searchformInput = $('#supplier_search_box');
                ...
                success: function (result) {
                    ...
                    if (jsonData.length != 0) {
                        $.each(jsonData, function(index, val) {
                            ...
                            let editCellHtml = '<a class="btn btn-sm btn-info btn-block" href="suppliers/' + val.id + '/edit" data-toggle="tooltip" title="{{ trans("usersmanagement.tooltips.edit") }}">{!! trans("usersmanagement.buttons.edit") !!}</a>';
                            let deleteCellHtml = '<form method="POST" action="/suppliers/'+ val.id +'" accept-charset="UTF-8" data-toggle="tooltip" title="Delete">' +
                           ...
                           '<td>' + val.suppliers_id + '</td>' +
                                '<td>' + val.company_name + '</td>' +
                                '<td class="hidden-xs">' + val.company_suppliers_name + '</td>' +
                                '<td class="hidden-xs">' + val.suppliers_phone + '</td>' +
                                '<td class="hidden-xs">' + val.suppliers_payment_method + '</td>' +
                                '<td class="hidden-xs">' + val.suppliers_final_rate + '</td>' +
        ...
        searchformInput.keyup(function(event) {
            if ($('#supplier_search_box').val() != '') {
                clearSearchTrigger.show();
            } else {
                clearSearchTrigger.hide();
                resultsContainer.html('');
                usersTable.show();
                cardTitle.html("{!! 'Showing All Suppliers' !!}");
                userPagination.show();
                usersCount.html(" ");
            };
        });
